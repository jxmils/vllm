...
#SBATCH --job-name=r32_sp128_sd128_pp2_tp2_qwen3_nsys
#SBATCH --nodes=2
...
# Learned from the successful ARC TP=2 Ray-worker Nsight run:
# - Ray must be installed in the active venv.
# - RAY_TMPDIR must be short (/tmp/vray-...) to avoid AF_UNIX socket path limits.
# - Ray worker .nsys-rep files may stay 0 bytes until Nsight finalizes; wait up to 900s.
# - Copy reports while Ray is still alive, then export nsys stats automatically.

SCRIPT_VERSION="arc-ray-qwen3-30b-a3b-r32-sp128-sd128-tp2-pp2-ray-worker-nsys-learned-v4"
...
WORKER_NSYS_FINALIZE_POLL_S="${WORKER_NSYS_FINALIZE_POLL_S:-15}"
WORKER_NSYS_FLUSH_WAIT_S="${WORKER_NSYS_FLUSH_WAIT_S:-900}"
MIN_WORKER_NSYS_REP_BYTES="${MIN_WORKER_NSYS_REP_BYTES:-4096}"
EXPECTED_WORKER_NSYS_REPS_PER_NODE="${EXPECTED_WORKER_NSYS_REPS_PER_NODE:-}"
NSYS_STATS_ENABLE="${NSYS_STATS_ENABLE:-1}"
NSYS_STATS_REPORTS="${NSYS_STATS_REPORTS:-cuda_gpu_kern_sum,cuda_api_sum,cuda_gpu_mem_time_sum,nvtx_sum}"
NSYS_STATS_TOPK_LINES="${NSYS_STATS_TOPK_LINES:-20}"
NSYS_STATS_FORCE_REEXPORT="${NSYS_STATS_FORCE_REEXPORT:-1}"
...
wait_for_real_worker_nsys_reports() {
  ...
  local poll_s="${2:-15}"
  local elapsed=0
  local expected_per_node="${EXPECTED_WORKER_NSYS_REPS_PER_NODE:-}"
  ...
  if [ -z "${expected_per_node}" ]; then
    expected_per_node="${GPUS_PER_NODE:-1}"
  fi
  ...
  echo "[nsight-copy] expected non-empty worker reports per node: ${expected_per_node}"
  echo "[nsight-copy] minimum report size: ${MIN_WORKER_NSYS_REP_BYTES} bytes"
  ...
  while [ "${elapsed}" -lt "${timeout_s}" ]; do
    ...
    for node in ${HEAD_NODE} ${WORKER_NODES}; do
      ...
      echo "[nsight-copy] node=${node} non-empty worker reports: ${count}/${expected_per_node}"

      if [ "${count:-0}" -lt "${expected_per_node}" ]; then
        all_ok=0
        missing_nodes="${missing_nodes} ${node}(${count:-0}/${expected_per_node})"
      fi
    done

    echo "[nsight-copy] current copied worker Nsight files:"
...
generate_worker_nsys_stats() {
  set +e

  if [ "${NSYS_STATS_ENABLE:-1}" != "1" ]; then
    echo "[nsys-stats] NSYS_STATS_ENABLE=${NSYS_STATS_ENABLE}; skipping worker report stats."
    return 0
  fi

  if ! command -v nsys >/dev/null 2>&1; then
    echo "[nsys-stats] WARNING: nsys not found in PATH; cannot generate stats."
    return 0
  fi

  local stats_dir="${TRACE_RUN_DIR}/nsys_stats"
  mkdir -p "${stats_dir}"

  echo "[nsys-stats] generating reports into ${stats_dir}"
  echo "[nsys-stats] reports=${NSYS_STATS_REPORTS}"
  ...
  while IFS= read -r -d '' rep; do
    ...
    echo "[nsys-stats] processing ${rep}"

    if [ "${NSYS_STATS_FORCE_REEXPORT:-1}" = "1" ]; then
      rm -f "${rep%.nsys-rep}.sqlite" 2>/dev/null || true
    fi

    nsys stats \
      --force-overwrite=true \
      --report "${NSYS_STATS_REPORTS}" \
      --output "${stats_dir}/${base}" \
      "${rep}" \
      2>&1 | tee "${stats_dir}/${base}.stats.log" || true
  done < <(
    find "${TRACE_RUN_DIR}/ray_worker_nsight" -type f \
      -name '*worker_process*.nsys-rep' \
      ! -name '*empty*' \
  ...
  if [ "${found}" != "1" ]; then
    echo "[nsys-stats] no non-empty worker_process_*.nsys-rep files found."
    return 0
  fi

  echo "[nsys-stats] generated files:"
  find "${stats_dir}" -type f -printf '[nsys-stats]   %p %s bytes\n' 2>/dev/null | sort || true

  echo "[nsys-stats] top CUDA kernels from each worker:"
  for csv in "${stats_dir}"/*_cuda_gpu_kern_sum.csv; do
    [ -f "${csv}" ] || continue
    echo "[nsys-stats] ===== $(basename "${csv}") ====="
    head -n "${NSYS_STATS_TOPK_LINES}" "${csv}" | sed 's/^/[nsys-stats]   /' || true
  done
}
...
echo "EXPECTED_WORKER_NSYS_REPS_PER_NODE=${EXPECTED_WORKER_NSYS_REPS_PER_NODE:-<GPUS_PER_NODE>}"
echo "NSYS_STATS_ENABLE=${NSYS_STATS_ENABLE}"
echo "NSYS_STATS_REPORTS=${NSYS_STATS_REPORTS}"
...
export EXPECTED_WORKER_NSYS_REPS_PER_NODE='${EXPECTED_WORKER_NSYS_REPS_PER_NODE}'
...
echo "Generating Nsight stats from copied worker reports..."
generate_worker_nsys_stats || true

echo "NCCL log summary:"
ls -lh "${TRACE_RUN_DIR}"/nccl_logs 2>/dev/null || true
grep -hE 'AllReduce|AllGather|ReduceScatter|Broadcast|Send|Recv|NVLS|P2P|Channel|Connected|transport|opCount' \
  "${TRACE_RUN_DIR}"/nccl_logs/*.log 2>/dev/null | head -300 || true
